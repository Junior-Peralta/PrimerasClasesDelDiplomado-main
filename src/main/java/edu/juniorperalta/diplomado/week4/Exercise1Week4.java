package edu.juniorperalta.diplomado.week4;

import edu.juniorperalta.diplomado.model.week4.IceScream;
...
public class Exercise1Week4 {
    ...
    public IceScream createIceScreamCombination(List<IceScream> iceScreams, Double combinationCost) {
        return IceScream.builder()
                .id("MyID")
                .name(generateIceCreamName(iceScreams))
                .flavor("MIXTO")
                .amount(calculateIceCreamPrice(iceScreams, combinationCost))
                .build();
    }

    private BigDecimal calculateIceCreamPrice(List<IceScream> iceScreams, Double combinationCost) {
        ...
        for(IceScream iceScream : iceScreams){
            if(result.doubleValue() == -1){
                ...
                BigDecimal decimalValueOfPercentage = new BigDecimal(combinationCost).divide(new BigDecimal(100), 2, RoundingMode.CEILING);
    ...
    private String generateIceCreamName(List<IceScream> iceScreams) {
        iceScreams.sort((o1, o2) -> Double.compare(o2.getAmount().doubleValue(), o1.getAmount().doubleValue()));
